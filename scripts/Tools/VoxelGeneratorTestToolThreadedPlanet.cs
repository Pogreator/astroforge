...
[Tool]
public partial class VoxelGeneratorTestToolThreadedPlanet : MeshInstance3D
{
	...
	private async void RunVoxelGenerationTest()
	{
		...
		long noiseGroupId = WorkerThreadPool.AddGroupTask(Callable.From<int>((index) => 
		{
			...
			byte[] rawBytes = PlanetGenerator.GenerateChunkDataPlanet(relativePos, Vector3.Zero, WorldSeed, (float)(GenerationRadius - 0.5) * 16.0f);
